...
class H2Client:
	...
	# def tls_setup_exchange(self, dn, port, use_insecure_ciphers=True):# 获取目标地址信息
	# 	addr_info = socket.getaddrinfo(dn, port, socket.AF_UNSPEC, socket.SOCK_STREAM)
	...
	def tls_setup_exchange(self, dn, port, use_insecure_ciphers = True):
		...
		try:
			...
			logging.error(f"[ssl_sock.connect(ip_and_port)]: Exception occurred: {e}")
			logging.error(traceback.format_exc())

		print("@ PASS 4")

		print("ssl_sock.selected_alpn_protocol():", ssl_sock.selected_alpn_protocol())

		# try:
		# 	assert('h2' == ssl_sock.selected_alpn_protocol())
		# except Exception as e:
		# 	logging.error(f"[assert('h2' == ssl_sock.selected_alpn_protocol())]: Exception occurred: {e}")
		# 	logging.error(traceback.format_exc())
	...
	def send_sequence(self, frames=None):
		if not frames:
			...
			isSend = self.sock.send(sequence)
			# print("^^^^^^^^^^^^^^^^^^^")
			# print("[Send]:")
			# print("isSend", isSend)
...
if __name__ == '__main__':
    ...
    h2client.send("118.31.168.136", 93, "118.31.168.13")
